...
export default function ModalExpCampana({ detalle, onCerrar, onRegistrarAvance }) {
  const { user } = useAuth()
  ...
  const guardar = async () => {
    ...
    setGuardando(true)
    const ok = await onRegistrarAvance({ estadoNuevo, nota, registradoPor: user?.nombre || 'Asesora' })
    setGuardando(false)
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 14, width: '100%', maxWidth: 620, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>{lead.nombre}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {envios.map((e) => (
                <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 12 }}>
                  <span style={{ minWidth: 90, color: 'var(--text-primary)', fontWeight: 600 }}>{nombreCorreo(e.correo_numero)}</span>
