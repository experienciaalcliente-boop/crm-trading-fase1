    ...
    <div style={{ padding:24, maxWidth:1200, margin:'0 auto' }}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:8 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <span style={{ fontSize:11, color:'#3d5070' }}>Act: {format(d.lastUpdate, 'HH:mm:ss')}</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:16 }}>
        ...
        <div className="crm-card" style={{ padding:18 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#7a8aaa', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:14 }}>Distribución de riesgo</div>
          {[
            ...
            <div key={label} style={{ marginBottom:12 }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                ...
                <span style={{ fontSize:13, fontWeight:700, color }}>{value} <span style={{ color:'#506080', fontWeight:400, fontSize:11 }}>({pct}%)</span></span>
              </div>
              <div style={{ height:6, background:'rgba(255,255,255,0.06)', borderRadius:3, overflow:'hidden' }}>
        ...
        {/* Último contacto */}
        <div className="crm-card" style={{ padding:18 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#7a8aaa', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:14 }}>Último contacto efectivo</div>
          {[
            ...
            { label:'Sin contacto registrado',value: d.segContacto.sinContacto, color:'#506080' },
        ...
        <div className="crm-card" style={{ padding:18, overflowY:'auto', maxHeight:220 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#7a8aaa', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:10 }}>Alumnos en riesgo alto</div>
          {d.alumnosRiesgoAlto.length === 0 ? (
            <div style={{ color:'#3d5070', fontSize:12, textAlign:'center', padding:'20px 0' }}>✓ Sin alumnos en riesgo alto</div>
            ...
            <div key={al.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'5px 0', borderBottom:'1px solid rgba(255,255,255,0.04)' }}>
              <div>
                <div style={{ fontSize:12, fontWeight:600, color:'#e2e8f4' }}>{al.nombre}</div>
                <div style={{ fontSize:10, color:'#506080' }}>{al.programa}</div>
              </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
        {/* Dona pipeline */}
        <div className="crm-card" style={{ padding:18 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#7a8aaa', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:14 }}>Distribución actual (historial completo)</div>
        ...
        <div className="crm-card" style={{ padding:18 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#7a8aaa', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:14 }}>Pipeline por programa</div>
        ...
        <table className="crm-table">
          ...
          <tbody>
            {d.desempenoPorAsesora.map((a, i) => (
              <tr key={a.asesora}>
                <td style={{ fontWeight:600, color:'#e2e8f4' }}>{a.asesora}</td>
                ...
                <td>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    <span style={{ fontSize:12, fontWeight:600, color:'#e2e8f4', minWidth:32 }}>{a.contactabilidad}%</span>
                  </div>
                </td>
                ...
                <td style={{ textAlign:'center' }}>
                  {a.riesgoAlto > 0 ? <span style={{ color:'#f87171', fontWeight:700 }}>{a.riesgoAlto}</span> : <span style={{ color:'#3d5070' }}>—</span>}
                </td>
                <td style={{ textAlign:'center' }}>
                  {a.sinContacto7 > 0 ? <span style={{ color:'#fb923c', fontWeight:700 }}>{a.sinContacto7}</span> : <span style={{ color:'#3d5070' }}>—</span>}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:16, marginBottom:16 }}>
        ...
        <div className="crm-card" style={{ padding:18 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#7a8aaa', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:6 }}>Capital real (USD)</div>
          <div style={{ fontSize:11, color:'#3d5070', marginBottom:10 }}>Total: <span style={{ color:'#7ab3ff', fontWeight:700 }}>{d.totalCuentasReales}</span></div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
        ...
        <div className="crm-card" style={{ padding:18 }}>
          <div style={{ fontSize:12, fontWeight:700, color:'#7a8aaa', textTransform:'uppercase', letterSpacing:'0.06em', marginBottom:4 }}>Resumen en Dólares</div>
          <div style={{ fontSize:10, color:'#3d5070', marginBottom:12 }}>Para alumnos con cuota en USD</div>
          {[
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:16, marginBottom:16 }}>
        <div className="crm-card" style={{ padding:18 }}>
          ...
          {d.motivosFrecuentes.length === 0 ? <div style={{ color:'#3d5070', fontSize:13 }}>Sin datos</div>
          ...
          {Object.entries(d.herramientas).map(([tool, count], i) => (
            <div key={tool} style={{ marginBottom:10 }}>
              <div style={{ display:'flex', justifyContent:'space-between', marginBottom:4 }}>
                ...
                <span style={{ fontSize:12, fontWeight:600, color:'#e2e8f4' }}>{count} <span style={{ color:'#506080', fontWeight:400 }}>({d.sesionesConcretadas>0?Math.round(count/d.sesionesConcretadas*100):0}%)</span></span>
              </div>
        ...
        <div className="crm-card" style={{ padding:18 }}>
          ...
          {d.sesionesPorPrograma.length === 0 ? <div style={{ color:'#3d5070', fontSize:13 }}>Sin datos</div>
