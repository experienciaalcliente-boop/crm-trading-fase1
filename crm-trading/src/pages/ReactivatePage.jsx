  ...
  const enviarCierre = async () => {
    ...
    try {
      const res = await fetch('/api/reactivate-forzar-envio', {
        ...
        body: JSON.stringify({ campana: 'reactivate-cierre' }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Error al enviar el correo de cierre')
      toast.success(`Cierre: ${data.enviados} enviados${data.errores ? `, ${data.errores} con error` : ''}. Quedan ${data.pendientesRestantes} pendientes.`)
      await r.cargar()
    } catch (err) {
      toast.error(err.message)
    } finally {
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            ...
            title="Envía el correo de cierre (pagar solo el saldo pendiente) a quien todavía no lo recibió"
            style={{
              display: 'flex', alignItems: 'center', gap: 6, padding: '7px 16px', borderRadius: 8,
